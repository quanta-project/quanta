...
Provides utilities for managing copyright headers of `The Quanta Core
...
---------------------------------------------------------
Updates all the copyright headers of `The Quanta Core developers` which were
changed in a year more recent than is listed. For example:
```
// Copyright (c) <firstYear>-<lastYear> The Quanta Core developers
```
will be updated to:
```
// Copyright (c) <firstYear>-<lastModifiedYear> The Quanta Core developers
```
...
those cases:
```
// Copyright (c) <year> The Quanta Core developers
```
will be updated to:
```
// Copyright (c) <year>-<lastModifiedYear> The Quanta Core developers
```
where the update is appropriate.

copyright\_header.py insert \<file\>
------------------------------------
Inserts a copyright header for `The Quanta Core developers` at the top of the
...
If the file already has a copyright for `The Quanta Core developers`, the
script will exit.
...
Generates a quanta.conf file in `share/examples/` by parsing the output from `quantad --help`. This script is run during the
release process to include a quanta.conf with the release binaries and can also be run by users to generate a file locally.
