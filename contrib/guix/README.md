# Bootstrappable Quanta Core Builds

This directory contains the files necessary to perform bootstrappable Quanta
Core builds.
...
To build Quanta Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Quanta Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/quanta-detached-sigs> ./contrib/guix/guix-codesign
```
