...
Below are some notes on how to build Quanta Core for Windows.

The options known to work for building Quanta Core on Windows are:
...
Note that for WSL the Quanta Core source path MUST be somewhere in the default mount file system, for
example /usr/src/quanta, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\quanta`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/quanta
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/quanta --strip
```
