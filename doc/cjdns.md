# CJDNS support in Quanta Core

It is possible to run Quanta Core over CJDNS, an encrypted IPv6 network that
...
done outside of Quanta Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quanta network and individual nodes.
...
before it will work with your Quanta Core node. This is described in steps
...
## Run Quanta Core with CJDNS

Once you are connected to the CJDNS network, the following Quanta Core
...
When enabled, this option tells Quanta Core that it is running in an
...
IPv6 local network. This helps Quanta Core perform better address management:
...
CJDNS support was added to Quanta Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `quanta-cli -addrinfo` to see the
...
## CJDNS-related information in Quanta Core

There are several ways to see your CJDNS address in Quanta Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quanta-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quanta-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `quanta-cli help getnodeaddresses` for details.
