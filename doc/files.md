...
The data directory is the default location where the Quanta Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.quanta/`
macOS    | `$HOME/Library/Application Support/Quanta/`
Windows  | `%LOCALAPPDATA%\Quanta\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `quanta.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Quanta blocks (dumped in network format, 128 MiB per file)
...
`./`               | `quanta.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `quantad` or `quanta-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `quantad.pid`        | Stores the process ID (PID) of `quantad` or `quanta-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `quantad` or `quanta-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [quanta.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
These subdirectories and files are no longer used by Quanta Core:
