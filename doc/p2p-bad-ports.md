When Quanta Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Quanta node is listening,
or one where a service is listening that is not related to the Quanta network.
As a result, this service may occasionally get connection attempts from Quanta
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Quanta Core,
trying to connect because it thinks there is a Quanta node on that
...
Below is a list of "bad" ports which Quanta Core avoids when choosing a peer to
