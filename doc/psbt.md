# PSBT Howto for Quanta Core

Since Quanta Core 0.17, an RPC interface exists for Partially Signed Quanta
...
PSBT is an interchange format for Quanta transactions that are not fully signed
...
Overall, the construction of a fully signed Quanta transaction goes through the
following steps:
...
- An **Extractor** produces a valid Quanta transaction (in network format)
...
#### Multisig with multiple Quanta Core instances
