...
The Quanta Core daemon can be configured to act as a trusted "border
router", implementing the quanta wire protocol and relay, making
...
The ZeroMQ feature in Quanta Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ quantad -zmqpubhashtx=tcp://127.0.0.1:28332 \
...
From the perspective of quantad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into quantad directly. Furthermore, no information is
...
using. Quantad appends an up-counting sequence number to each
