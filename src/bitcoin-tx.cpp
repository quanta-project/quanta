    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                ...
                "quanta-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded quanta transaction, or with \"-create [commands]\" to create a hex-encoded quanta transaction.\n"
                "\n"
