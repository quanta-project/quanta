...
BOOST_AUTO_TEST_CASE(rpc_rawsign)
{
    ...
    // Replace this with a valid Quanta address
    r = CallRPC(std::string("createrawtransaction ") + prevout + " " +
                "{\"QbzTVnp6vye7p9NobWEa3MHDpnfXfH6Bpn\":11}");
    std::string notsigned = r.get_str();

    // ✅ Quanta-compatible WIFs (base58Prefixes[SECRET_KEY] = {0xD2})
    std::string privkey1 = "\"QVm4LhAvJHqN9b7XjZPZxrSzzW1LccZnmwn5F2T4nuvSHmuA9d9h\"";
    std::string privkey2 = "\"QTRoNSCqZGBzhxFwMwPsvKg1s8c9ZUKqtzzHgQqUuwjxTUXm5EzZ\"";

    // First try: no keys → should not be complete
    r = CallRPC(std::string("signrawtransactionwithkey ") + notsigned + " [] " + prevout);
    BOOST_CHECK(r.get_obj().find_value("complete").get_bool() == false);

    // Second try: both keys → should be complete
...
BOOST_AUTO_TEST_CASE(help_example)
{
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", args), "> quanta-cli -named test foo=bar b=true n=1\n");
    ...
    // test shell escape
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b'ar"}}), "> quanta-cli -named test foo='b'''ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b\"ar"}}), "> quanta-cli -named test foo='b\"ar'\n");
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"foo", "b ar"}}), "> quanta-cli -named test foo='b ar'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", obj_value}}), "> quanta-cli -named test name='{\"foo\":\"bar\",\"b\":false,\"n\":1}'\n");
    ...
    BOOST_CHECK_EQUAL(HelpExampleCliNamed("test", {{"name", arr_value}}), "> quanta-cli -named test name='[\"bar\",false,1]'\n");
